...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Lioncoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Lioncoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Lioncoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Lioncoin 0.7 datadir."
    ;;
  3)
    echo "Detected Lioncoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Lioncoin 0.8 datadir."
    ;;
esac
